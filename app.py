    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                # Check if query contains a company name and KOSGEB support question
                query_lower = prompt.lower()
                if "kosgeb" in query_lower and any(word in query_lower for word in ["destek", "hibe", "teşvik"]):
                    ...
                    for i, word in enumerate(words):
                        if word.lower() in ["şirketi", "firması", "ltd", "a.ş.", "anonim"]:
                            if i > 0:
                                company_name = words[i-1]
                                # Check company eligibility
                                is_eligible, reason = check_company_eligibility(driver, company_name, llm)
                                if is_eligible is not None:
                                    if is_eligible:
                                        response = f"{company_name} şirketi KOSGEB desteklerine uygundur. Sebep: {reason}"
                                    else:
                                        response = f"{company_name} şirketi KOSGEB desteklerine uygun değildir. Sebep: {reason}"
                                else:
                                    response = reason
                                break
                
                # If no company eligibility check was performed, proceed with normal search
                if 'response' not in locals():
                    ...
                    prompt_template = PromptTemplate(
                        ...
                        Lütfen verilen bağlama ve konuşma geçmişine dayanarak net ve doğru bir yanıt ver. Eğer bir konudan emin değilsen veya bağlamda ilgili bilgi yoksa, bunu belirt."""
                    )
                    
                    if not relevant_docs:
                        response = "I apologize, but I don't have any relevant information in my database to answer your question. Please try uploading some documents first or rephrase your question."
                    else:
                        # Format the context from retrieved documents
                        context = "\n\n".join([f"Document {i+1} (Relevance: {doc['score']:.2f}):\n{doc['text']}" 
                                             for i, doc in enumerate(relevant_docs)])
